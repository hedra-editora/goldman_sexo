...
\addcontentsline{toc}{chapter}{Introdução, \textit{por Mariana Lins}}
...
não é correto dizer que a liberdade só poderá ser alcançada com a implementação do anarquismo.
...
qualidade dolorosamente profética de uma outra declaração sua, do 
mesmo texto: ``Os novos autocratas da Rússia desacreditaram os
...
utilizados em nome da moralidade, sob a escusa de colocá-las de volta ao eixo.
...
pessoas que são desprovidas de propriedades {[}\ldots{]} A moralidade da propriedade estabelece que essa
...
sua \textit{independência}. O~problema é que nesses casos, a condição de
...
estivesse crescendo de modo mais do que significativo nas estatísticas,\footnote{Ver os dados expostos no capítulo ``Casamento e amor.''}
ainda implicava uma
...
mencionada acima, de Henry Clay Frick por Alexander Berkman.\footnote{Elizabeth \textsc{s}.\,Lobo. \textit{Emma Goldman: a vida como revolução}. São Paulo: Editora Brasiliense.}
...
prostituição como \textit{a consequência direta de uma remuneração
...
disse do seu tempo, poderia ter dito do nosso: a \textit{cegueira deliberada}
...
acerca do \textit{verdadeiro método de prevenção}, quando a despeito dos
...
autobiografia \textit{Living my life}, a sua resposta não poderia ser mais
...
dessa deu-se por vencido. Anos depois desse encontro, Goldman parece
...
redução da mulher à condição de mercadoria sexual exigisse, antes, a repressão sexual: 
...
O problema é que a perda da virgindade \textit{sem a sanção da
...
\begin{quote}
Não importa o quanto mude, sempre dá ao homem direito e poder sobre 
sua esposa, não apenas sobre o seu corpo, mas também sobre suas 
ações, seus desejos; na verdade, sobre a
...
tenha, há muito, \textit{cessado de viver uma com a outra} --- esse, e não o
...
louvam a maternidade. Não houve crime maior contra a 
mulher do que esse.\footnote{Ver ``Os aspectos sociais do controle de natalidade'', p.\,\pageref{aborto}.}
\end{quote}
...
Em ``A tragédia da mulher emancipada'', porém, Goldman irá rememorar uma
...
``à condição de pagã, apta ao empalhamento'' pelas ``discípulas dessa emancipação pura e
...
respeito ao seu sistema reprodutivo: ``Ademais, é consenso entre 
os médicos mais sérios que a reprodução constante da parte das 
mulheres resulta no que em termos leigos é
chamado `problemas femininos'\,.''\footnote{Ver ``Os aspectos 
sociais do controle de natalidade'', p.\,\pageref{saude}.} Na
...
ao escrever logo na primeira página: ``A máxima memorável de Nietzsche, \textit{Quando 
você encontrar uma mulher, leve o chicote}, é considerada muito brutal e, no entanto, Nietzsche
...
Que Louise Michel não fosse, segundo Goldman, uma \textit{uraniania}, é
...
dar}; \textit{compre barato e venda caro}; \textit{uma mão suja lava a outra}.\footnote{Ver p.\,\pageref{suja}.}
\end{quote}
...
Para a anarquista é preciso ser universal o suficiente para não transformar
...
A questão é que se a luta efetivamente se der em nome da
